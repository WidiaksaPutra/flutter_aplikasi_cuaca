...
class IsiCardInformasiMaxLandscape extends StatelessWidget with sizeDevice {
  final String nameLokasi, icon, mainCuaca, dateTime, temperatur, windSpeed, humidity, clouds, jamNow;
  const IsiCardInformasiMaxLandscape({ Key? key, required this.nameLokasi, required this.icon, 
  ...
  Widget build(BuildContext context) {
    return CardInformasiCuaca(
      child: SizedBox(
        width: (sizeDevice.sizeWidth - getHaight(230)).w,
        height: sizeDevice.sizeHeight.h,
        child: Column(
          ...
          children: [
            ...
            ? Padding(padding: EdgeInsets.symmetric(horizontal: getWidth(3).w), child: Text("Updated $jamNow min ago", style: TextStyle(fontSize: 12.sp, color: whiteColor, fontWeight: FontWeight.bold, fontFamily: "OpenSauceSans") ,textAlign: TextAlign.center))
            : Container(
              ...
              child: Padding(padding: EdgeInsets.symmetric(vertical: getWidth(3).w), child: Text("Updated $jamNow min ago", style: TextStyle(fontSize: 12.sp, color: whiteColor, fontWeight: FontWeight.bold, fontFamily: "OpenSauceSans") ,textAlign: TextAlign.center)),
            ),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
              (getWidthVisiblePortrait() == false) ? const Text("") : Image.network('http://openweathermap.org/img/wn/$icon@2x.png'),
              Column(
                children: [
                  Row(
                    ...
                    children: [
                      Text(" $temperatur", 
                        style: TextStyle(fontSize: (getHeightVisiblePortrait() == false) ? 50.sp : 100.sp, 
                          color: whiteColor, 
                      ...
                      Text("°", style: TextStyle(fontSize: (getHeightVisiblePortrait() == false) ? 20.sp : 60.sp, color: whiteShadowColor, fontWeight: FontWeight.bold, fontFamily: "OpenSauceSans")),
                  ],),
            ...
            ContainerTopCuaca(icon: icon, speed: windSpeed, humidity: humidity, clouds: clouds),
          ],
        ),     
      ),
    );
